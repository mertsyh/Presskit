(function () {
  const STRINGS = {
    tr: {
      ...
      'empty.desc': 'Bu sayfa henüz bir press kit JSON dosyası içe aktarılmamış. Admin panelinden bir JSON dosyası içe aktarın.',
      'empty.link': 'Admin paneline git ↗',
      'kv.genre': 'Tür',
    ...
    en: {
      ...
      'empty.desc': 'No press kit JSON has been imported into this page yet. Import a JSON file from the admin panel.',
      'empty.link': 'Go to admin panel ↗',
      'kv.genre': 'Genre',
      'heading.platforms': 'Platform',
    ...
    container.innerHTML = valid.map((i, idx) => {
      ...
      <figure>
        <a href="${esc(i.url)}" target="_blank" rel="noopener" data-idx="${idx}">
          <img src="${esc(i.url)}" alt="${esc(caption)}" loading="lazy">
        </a>
  ...
  // ---------- data loading: committed data.json first, then local import fallback ----------
  async function loadData() {
    ...
    } catch (e) { /* no committed data.json yet, or not served over http(s) */ }
    try {
      const local = localStorage.getItem('presskitData');
      if (local) {
  ...
  function showEmptyState() {
    document.querySelector('.wrap').innerHTML = `
      <div class="empty-state">
        <h1>${esc(t('empty.title'))}</h1>
        <p>${esc(t('empty.desc'))}</p>
        <p><a class="btn btn-primary" href="admin.html">${esc(t('empty.link'))}</a></p>
      </div>`;
